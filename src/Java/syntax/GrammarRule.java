...
/**
 * A class representing a PCFG Grammar rule, that is: LHS -> RHS1 RHS2 ... RHSN
 * along with a probability
 * 
 * The specific format for the rule is: - non-lexical rule: LHS -> RHS1 RHS2 ...
 * RHSN[tab][tab]PROBABILITY - lexical rule LHS -> * word[tab][tab]PROBABILITY
 * 
 * @author Dave Kauchak
 * @version 10/5/2011
 * 
...
public class GrammarRule {
	private static final Pattern PCFG = Pattern
			.compile("(\\S+) -> (.*?)\\s*\t+([\\d\\.Ee-]+)");
	private static final String LEXICAL_PREFIX = "* ";

	private String lhs; // the left hand side symbol
	private ArrayList<String> rhs = new ArrayList<String>(); // the set of right
																// hand side
																// symbols
	private double weight = 0.0; // the weight (probability) of this rule,
									// initially 0.0
	private boolean lexical = false;

	/**
	 * Create a new Grammar rule with left as the LHS component, the right hand
	 * side components rhs and a probability of zero
	 * 
	 * @param left
	 *            the left hand side of the rule
	 * @param rhs
	 *            the right had side elements of the rule
	 */
	...
	/**
	 * Create a new Grammar rule with specification of whether or not it is a
	 * lexical rule
	 * 
	 * @param left
	 *            the LHS symbol
	 ...
	 *            the RHS symbols
	 * @param isLexical
	 *            whether this is a lexical rule or not
	 */
		...
		if (isLexical) {
			if (rhs.size() != 1) {
				throw new RuntimeException(
						"Tried to make a lexical PCFG with multiple children");
			}
	...
	/**
	 * Create a new Grammar rule with left as the LHS component, the right hand
	 * side components rhs and a weight (probability)
	 * 
	 * @param left
	 *            the left hand side of the rule
	 * @param rhs
	...
	/**
	 * Create a new Grammar rule directly from a String. See notes at the type
	 * of this file for the formatting of the string.
	 * 
	 * @param pcfgString
	 */
	 ...
	 * 
	 * @param weight
	 *            the weight (probability) associated with this rule
	 */
	...
	/**
	 * A string representation of this node using arrow notation, e.g.: S -> NP
	 * VP
	 */
	...
	/**
	 * Checks if this rule is equal to another object (most frequently another
	 * PCFGRule). Again We'll use string equality, though the results would be
	 * the same if we did it element-wise.
	 */
